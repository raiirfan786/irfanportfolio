      ...
      <main className="relative z-10 pt-32 pb-24 px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-24">
          {packages.map((pkg, idx) => (
            <motion.div
              ...
              className={`rounded-3xl border p-8 flex flex-col justify-between relative bg-gradient-to-br ${pkg.color} ${pkg.popular ? "shadow-xl shadow-purple-500/5 ring-1 ring-purple-500/40" : ""}`}
            >
              {pkg.popular && (
                ...
                <div className="border-t border-white/5 pt-6 mb-8">
                  <ul className="space-y-3.5">
                    {pkg.features.map((feature, featureIdx) => (
                      <li key={featureIdx} className="flex items-start space-x-3 text-xs text-white/70">
              ...
              <div className="pt-4">
                <MagneticButton className="w-full">
                  <Link
                    ...
                    className={`block w-full text-center py-3.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all border ${pkg.popular ? "bg-white text-black hover:bg-purple-500 hover:text-white border-white" : "bg-white/5 text-white hover:bg-white/10 border-white/10"}`}
                  >
                    {pkg.cta}
                  </Link>
                </MagneticButton>
        ...
        <div className="border-t border-white/5 pt-24 mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {pillars.map((pillar, idx) => (
              <div key={idx} className="p-6 rounded-2xl glass-panel border-white/5 bg-[#0b0b0b]/40 flex space-x-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/50 leading-relaxed font-normal">{pillar.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="border-t border-white/5 pt-24 max-w-4xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl font-extrabold text-white tracking-tight font-display">Frequently Asked Questions</h2>
          </div>

          <div className="space-y-4">
            {faqs.map((faq, idx) => (
              <div key={idx} className="border-b border-white/5 pb-4 transition-all">
                <button
                ...
                {activeFaq === idx && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="text-xs text-white/50 leading-relaxed pr-8">
                    {faq.a}
                  </motion.div>
                )}
              </div>
